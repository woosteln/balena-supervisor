...
describe('LocalModeManager', () => {
	...
	let containerIdFile: tmp.FileResult;
	let db: DB;
	...
	before(async () => {
		...
		containerIdFile = tmp.fileSync();
		fs.writeFileSync(containerIdFile.name, supervisorContainerId);
	...
	describe('engine snapshots collection', () => {
		...
		describe('local mode switch', () => {
			...
			describe('with supervisor being updated', () => {
				...
				it('does not conduct a suicide', async () => {
					const removeStubs = stubEngineObjectMethods(false);
	...
	after(async () => {
		sinon.restore();
		containerIdFile.removeCallback();
